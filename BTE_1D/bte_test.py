...
def bte_test(x,mu,w,k,vt0,vt1,Nx,Ns,Nk,Np,L,Tr,dT,a,index,path,device):
	...
	net0.load_state_dict(torch.load(path+"model0.pt",map_location=device)['state_dict'])
	net0.eval()
	...
	for j in range(len(L)):
		for i in range(Nx):
			...
			q[i,j] = torch.sum(sum_ve*C*wk*v**2/(4*np.pi)).cpu().data.numpy()/TC.cpu().data.numpy()*(10**L[j])
